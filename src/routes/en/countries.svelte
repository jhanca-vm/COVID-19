...
<section class="container mx-auto pt-1 md:grid md:grid-cols-2 md:col-gap-6 lg:-mt-5 xl:grid-cols-3">
	{#each countries as {countryInfo, country, todayCases, todayRecovered, todayDeaths, cases, deaths, recovered, tests, active, critical}}
		<div class="country grid grid-cols-2 bg-gray-100 rounded-lg shadow-md mb-6 max-w-md mx-auto lg:mb-8">
			...
			<div class="text-center py-2 h-32">
				...
				{#if todayCases == 0 && todayRecovered == 0 && todayDeaths == 0}	
					<p class="mt-2 truncate">No reports yet</p>
				{:else}
					<p class="mt-2 truncate">{nf.format(todayCases)} {#if todayCases == 1}case{:else} cases{/if}</p>
					<p class="truncate">{nf.format(todayRecovered)} {#if todayRecovered == 1}recovered{:else}recovereds{/if}</p>
					<p class="truncate">{nf.format(todayDeaths)} {#if todayDeaths == 1}death{:else}deaths{/if}</p>
				{/if}
			</div>
			<div id={countryInfo.iso3} class="hidden col-span-2 grid grid-cols-2 italic bg-gray-100 shadow-inner text-center py-4">
				<div class="px-4">
					<p class="truncate">{nf.format(cases)} {#if todayCases == 1}case{:else} cases{/if}</p>
					<p class="truncate">{nf.format(recovered)} {#if todayRecovered == 1}recovered{:else}recovereds{/if}</p>
					<p>{nf.format(deaths)} {#if todayDeaths == 1}death{:else}deaths{/if}</p>
				</div>
				<div class="px-4">
					<p class="truncate">{nf.format(tests)} {#if tests == 1}test{:else}tests{/if}</p>
					<p class="truncate">{nf.format(active)} {#if active == 1}active{:else}actives{/if}</p>
					<p class="truncate">{nf.format(critical)} {#if active == 1}critical{:else}criticals{/if}</p>
				</div>
			</div>
		</div>
	{/each}
